...
=======
>>>>>>> cbc6a5132f5c14ac1495b796a88431f63f170bc4
...
>>>>>>> cbc6a5132f5c14ac1495b796a88431f63f170bc4
/**
 * Implementation of {@link CheetahConnection}.
 */
class CheetahConnectionImpl implements CheetahConnection {
<<<<<<< HEAD
    ...
    CheetahConnectionImpl(String url, String coordinatorUrl) {
    ...
    private void parseField(List<String> fieldNames, List<ColumnMetaData.Rep> fieldTypes,
        ...
        if (i < 0) {
            /* commit added by edwardlol */
            // current column name in the result does not exsist in the data source
            // should it throw an exception here?
            /* end commit */
            return;
        }
    ...
    /**
     * doc added by edwardlol
     * expect a scalar field with a given name
...
//    }
    ...
    private Long extractTimestampField(JsonParser parser)
        ...
        try {
            ...
            // ignore bad value
            /* edited by edwardlol */
            /* add debug info, may change to log4j later */
            if (CalcitePrepareImpl.DEBUG) {
        ...
        try (InputStream in0 = post(url, jsonQuery, requestHeaders, 10000, 1800000);
            ...
            for (JsonSegmentMetadata o : list) {
                for (Map.Entry<String, JsonColumn> entry : o.columns.entrySet()) {
                    if (entry.getKey().equals(CheetahTable.DEFAULT_TIMESTAMP_COLUMN)) {
                        // timestamp column
                        continue;
                    }
                    final CheetahType cheetahType;
                    try {
                        cheetahType = CheetahType.valueOf(entry.getValue().type);
                    } catch (IllegalArgumentException e) {
                        // ignore exception; not a supported type
                        continue;
                    }
                    fieldBuilder.put(entry.getKey(), cheetahType.sqlType);
                }
    ...
    /**
     * Reads data source names from Cheetah.
     */
    Set<String> tableNames() {
        ...
        if (CalcitePrepareImpl.DEBUG) {
            System.out.println("Cheetah: table names" + data + "; " + url);
        }
        try (InputStream in0 = post(url, data, requestHeaders, 10000, 1800000);
             InputStream in = traceResponse(in0)) {
            final ObjectMapper mapper = new ObjectMapper();
    ...
    private static class JsonAggregator {
        ...
        CheetahType cheetahType() {
            if (type.startsWith("long")) {
                return CheetahType.LONG;
            }
            if (type.startsWith("double")) {
                return CheetahType.FLOAT;
            }
            if (type.equals("hyperUnique")) {
                return CheetahType.hyperUnique;
            }
    ...
    enum CheetahType {
        ...
        CheetahType(SqlTypeName sqlType) {
...
// End CheetahConnectionImpl.java
